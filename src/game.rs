...
//!
//! [`run`] is the whole program, and `src/main.rs` is four lines that call it.
//! That is Android's doing: an Android app has no `main`, it has a shared
//! object the activity loads and an `SDL_main` SDL calls, so the loop has to
//! be something a library can export. Splitting it here rather than writing a
//! second boot path keeps one loop for every platform — the same rule that
//! made the inspection tools subcommands instead of a second binary.

use crate::inspect;
use crate::install::binding::{Action as Control, Bindings, Sign, Trigger};
use crate::install::config::{Channel, Config, Flag, Sound};
use crate::install::engine::Settings;
use crate::install::progress::Progress;
use crate::install::save::FlagStore;
use crate::install::vfs::Vfs;
use crate::media::AudioBuffer;
use crate::media::ImageScaler;
use crate::playback::lipsync::compose_mouths;
use crate::playback::som::{self, Device};
use crate::ui::backlog;
use crate::ui::bar::{self, Bar};
use crate::ui::comment;
use crate::ui::dress;
use crate::ui::ending;
use crate::ui::menu::{Action, Menu, Mode, SaveState, Session, Showing, SystemSe};
use crate::ui::options::{self, Dir, Display, Som};
use crate::ui::paths::Paths;
use crate::ui::replay::{self, Scenes};
use crate::ui::saveload::{self, Slots};
use crate::ui::screen::{self, Layer, Resolution, Screen};
use crate::ui::select::{self, Choice, Input, Select};
use crate::{install::ini::Ini, playback::text, Mixer, Stage};
...
impl Sounds {
    ...
    fn get(&mut self, vfs: &Vfs, path: &str) -> Option<Arc<AudioBuffer>> {
        ...
        let buffer = vfs
            ...
            .and_then(|bytes| match crate::media::decode_audio(bytes) {
...
/// [`crate::playback::som`] for the levels themselves and where they come
/// from.
struct Pads {
...
impl Device for Pads {
    ...
    fn detect(&mut self) -> Option<usize> {
        ...
        (0..self.open.len().min(crate::ui::options::SOM_PORTS)).find(|port| self.open(*port))
    }
...
///
/// [`crate::install::binding`] is the table itself and knows nothing
...
struct Player<'a> {
    ...
    /// target at all. See `crate::playback::stage::apply_skip_flag`.
    skip_flag: bool,
...
impl Player<'_> {
    ...
    /// [`crate::install::engine::Settings::pixel_perfect`].
...
pub fn run_with(arguments: Vec<String>) -> Result<()> {
    ...
    log::info!("ffmpeg {}", crate::media::ffmpeg_version());
    ...
    crate::playback::scale::set_kernel(settings.ui_scaler.kernel());
    ...
    let mixer = Mixer::new();
    let spec = AudioSpec {
        freq: Some(crate::media::SAMPLE_RATE as i32),
        channels: Some(crate::media::CHANNELS as i32),
    ...
    let mut player = Player {
        ...
        flags: crate::install::save::load_flags(&game, &film),
    ...
    loop {
        ...
        if let Some(p) = progress.as_ref() {
            crate::playback::stage::apply_end_roll(&mut script, p.end_roll());
        }
        // And whether this script records a skip target at all, which is the
        // engine's skip flag as it stands *now*, at the load.
        crate::playback::stage::apply_skip_flag(&mut script, player.skip_flag);
        log::info!(
            "{} events, length {} ({:.1}s)",
            script.events.len(),
            script.length,
...
/// [`crate::ui::menu::Entry`] for the evidence; the short of it is that a
...
enum Outcome {
    ...
    /// [`crate::install::progress::Progress::back`] — and the script
...
/// save state — see [`crate::ui::ending`]. A card that will not load costs the
...
/// drives — see [`crate::install::config::Sound`].
fn apply_volumes(config: &Config, sound: Sound, mixer: &Mixer) {
    mixer.set_gains(crate::playback::mixer::Gains {
...
fn run_menu(
    ...
    loop {
        ...
        // paced the same way — see `crate::ui::dress::SLIDE_FRAMES`. The
        // dim is timed in milliseconds rather than in ticks, which is why the
        // frame's own length goes with it; see `crate::ui::menu::Dim`.
        ...
        for action in actions {
            match action {
                ...
                //
                // See `crate::ui::options::DisplayRequest`.
...
fn run_comment(
    ...
    let english = player.film.get_bool("UseEnglish").unwrap_or(false);
    let exe = match crate::install::storage::read(&player.executable) {
        Ok(bytes) => bytes,
        Err(err) => {
...
fn play_menu_bgm(player: &mut Player, path: Option<&str>) {
    ...
    let load = |handle| {
        player
            ...
            .read(handle)
            .ok()
            .and_then(|bytes| crate::media::decode_audio(bytes).ok())
            .map(Arc::new)
    };
...
/// is resampled to whatever it lands on by [`crate::playback::scale`], so
...
struct DressBackground<'r> {
    ...
    /// decoding.
    clip: Option<crate::media::VideoDecoder>,
    ...
    fn load(
        ...
        let (image, clip) = match chosen {
            ...
            dress::Background::Movie(path) => {
                let mut clip = crate::media::VideoDecoder::open(vfs.read_path(path)?)?;
                let frame = clip
                    .next_frame()?
    ...
    fn advance(&mut self) {
        ...
        while self.shown < wanted {
            match clip.next_frame() {
                ...
                // the top — see `crate::ui::dress::BACKGROUND_FPS`. The
...
fn run_script(
    ...
    // it cut to. See `crate::media::ImageScaler`.
    ...
    loop {
        ...
        // rather than at the next script. See `crate::playback::mixer`.
        ...
        // which is why it stops here too. See `crate::playback::som`.
        ...
        // SIMD. See `crate::media::VideoDecoder::set_output_size`. What is
